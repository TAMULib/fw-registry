...
var hegisPoQuery = 'WITH _payments AS ('
  ...
  + '\n\t\tLEFT JOIN folio_orders.order_invoice_relationship__t poir ON poir.invoice_id = fti.invoice_id'
  ...
  + '\n\t) agg'
  + '\n\tLEFT JOIN folio_orders.order_invoice_relationship__t poir2 ON poir2.invoice_id = agg.ii'
  + '\n),'
  + '\n_id AS ('
  + '\n\tSELECT *'
  ...
  + '\n)'
  + '\nSELECT DISTINCT ON (po.jsonb ->> \'poNumber\', poi.title)'
  + '\n\tpo.jsonb ->> \'poNumber\' AS po_number,'
  + '\n\tsubstring(ce.jsonb ->> \'code\', 9) AS ship_to,'
  + '\n\tpoi.title AS title,'
  + '\n\tpo.jsonb ->> \'workflowStatus\' AS po_status,'
  + '\n\tpo.jsonb->\'notes\'->0 AS line_item_notes,'
  + '\n\tpo.jsonb->\'ongoing\'->>\'interval\' AS renewal_interval,'
  + '\n\tregexp_replace(po.jsonb->>\'notes\', \'(?i)^.*hegis<(.*?)>.*$\', \'\\1\') AS hegis,'
  + '\n\t\'\' AS problems,'
  ...
  + '\n\tpol.jsonb->\'vendorDetail\'->\'referenceNumbers\'->0->>\'refNumber\' AS vendor_reference_number,'
  + '\n\t('
  ...
  + '\nLEFT JOIN folio_orders.po_line pol ON pol.purchaseorderid = po.id'
  + '\nLEFT JOIN _payments _p ON _p.purchase_order_id = po.id'
  + '\nLEFT JOIN folio_configuration.config_data ce ON ce.id = (po.jsonb ->> \'shipTo\') ::uuid'
  + '\nLEFT JOIN folio_derived.po_instance poi ON poi.po_number = po.jsonb ->> \'poNumber\' '
  + '\nWHERE po.jsonb->>\'notes\' ~ ' + hegisPattern
  + poStatus
