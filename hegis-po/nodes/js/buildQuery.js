...
var hegisPoQuery = 'WITH _payments AS ('
  ...
  + '\n\t\tFROM folio_reporting.finance_transaction_invoices fti'
  + '\n\t\tLEFT JOIN public.po_order_invoice_relns poir ON poir.invoice_id = fti.invoice_id'
  + '\n\t\tGROUP BY fti.invoice_id'
  + '\n\t) agg'
  + '\n\tLEFT JOIN public.po_order_invoice_relns poir2 ON poir2.invoice_id = agg.ii'
  + '\n),'
  + '\n_id AS ('
  + '\n\tSELECT *'
  ...
  + '\nLEFT JOIN public.po_lines pol ON pol.purchase_order_id = po.id'
  + '\nLEFT JOIN _payments _p ON _p.purchase_order_id = po.id'
  + '\nLEFT JOIN public.configuration_entries ce ON ce.id = po.ship_to'
  + '\nLEFT JOIN folio_reporting.po_instance poi ON poi.po_number = po.po_number'
